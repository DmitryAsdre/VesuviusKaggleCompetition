...
class CFG:
    ...
    PATH_TO_SAVE = '../models/'
    exp_name = 'segformer_b3_default_augs_512_512_conv2d'
    chans_idxs = [26, 27, 28, 29, 30, 31, 32, 33]
...
def train_fn(train_loader, model, criterion, optimizer, device):
    ...
    for step, (images, labels, _) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        if CFG.label_noise:
            angle = np.random.randint(1, 20) / 5
            labels = rotate(labels, angle)
        
        batch_size = labels.size(0)

        with autocast(CFG.use_amp):
            y_preds = model(images)[0]
            y_preds = F.interpolate(y_preds, (CFG.tile_size, CFG.tile_size), align_corners=False, antialias=False, mode='bilinear')
...
def train_on_fold(valid_img, writer):    
    ...
    model = torch.nn.Sequential(torch.nn.Conv2d(8, 3, kernel_size=1), 
