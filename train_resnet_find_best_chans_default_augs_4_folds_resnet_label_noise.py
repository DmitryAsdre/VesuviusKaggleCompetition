...
class CFG:
    ...
    exp_name = 'find_best_chans'
    chans_idxs = np.array([18, 19, 20])
...
def train_on_fold(valid_img, writer):    
    ...
    model = smp.Unet(encoder_name='resnet34', encoder_weights=CFG.encoder_weights, decoder_attention_type=CFG.decoder_attention_type)
...
if __name__ == "__main__":
    WRITER_FOLDER = './finding_best_chans_resnet_tensorboard'
    CFG.chans_idxs += 11
    
    for i in range(11, 25):
        CFG.chans_idxs += 1
        writer = SummaryWriter(log_dir=os.path.join(WRITER_FOLDER, f'resnet_{CFG.chans_idxs}'))
