...
from utils.unetplusplus_meanpooled import UnetPlusPlusMeanPooled
...
class CFG:
    ...
    PATH_TO_SAVE = '../models/'
    exp_name = 'resnet_101_unet_meanpooled_32_ch'
...
def train_fn(train_loader, model, criterion, optimizer, device):
    ...
    for step, (images, labels, _) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        loss /= CFG.agg_grad_size
        scaler.scale(loss).backward()

        if (step + 1) % CFG.agg_grad_size == 0 or (step + 1) == len(train_loader):
...
def train_on_fold(valid_img, writer):    
    ...
    model = UnetPlusPlusMeanPooled(encoder_name=CFG.backbone_name, crop_stride=CFG.crop_stride, encoder_weights=CFG.encoder_weights, in_channels=5)
    #MAnetMeanPooled(encoder_name=CFG.backbone_name, crop_stride=CFG.crop_stride, encoder_weights=CFG.encoder_weights)
    #UnetMeanPooled(encoder_name=CFG.backbone_name, decoder_attention_type=CFG.decoder_attention_type, 
