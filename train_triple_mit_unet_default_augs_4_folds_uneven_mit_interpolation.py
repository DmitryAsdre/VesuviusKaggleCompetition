...
class CFG:
    ...
    PATH_TO_SAVE = '../models/'
    exp_name = 'triple_mit_b1_b2_9_slices_from_the_middle_default_augmentations_4_folds_uneven_interpolate'
...
def train_fn(train_loader, model, criterion, optimizer, device):
    ...
    for step, (images, labels, _) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        images_interpolate = F.interpolate(images, size=(CFG.in_shape, CFG.in_shape), mode='bilinear', align_corners=False, antialias=True)
        labels_interpolate = F.interpolate(labels, size=(CFG.in_shape, CFG.in_shape), mode='bilinear', align_corners=False, antialias=True)
        ...
        with autocast(CFG.use_amp):
            y_preds = model(images_interpolate)
            loss = criterion(y_preds, labels_interpolate)
...
def valid_fn(valid_loader, model, criterion, img_shape, device):
    ...
    for step, (images, labels, valid_xyxy) in tqdm(enumerate(valid_loader), total=len(valid_loader)):
        ...
        with torch.no_grad():
            images = F.interpolate(images, size=(CFG.in_shape, CFG.in_shape), mode='bilinear', align_corners=False, antialias=True)
            y_preds = model(images)
            y_preds =  F.interpolate(y_preds, size=(CFG.out_shape, CFG.out_shape), mode='bilinear', align_corners=False, antialias=True)
