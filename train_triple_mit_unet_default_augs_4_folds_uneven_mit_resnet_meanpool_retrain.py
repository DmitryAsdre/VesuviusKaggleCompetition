...
from utils.pooled_unet import PooledResnet
...
class CFG:
    ...
    PATH_TO_SAVE = '../models/'
    exp_name = 'resnet_meanpool_18_retrain'
...
def retrain_on_fold(valid_img, writer, model_path):    
    ...
    train_images, train_masks, valid_images, valid_masks, valid_xyxys = get_train_valid_dataset_4_folds(valid_img, read_images_mask, CFG.tile_size, CFG.stride, CFG.stride_valid)
    ...
    model_state_dict = torch.load(model_path, map_location=torch.device('cuda:0'))
    model = PooledResnet()
    ...
    #optimizer = optimizer.to(CFG.device)
    #scheduler = scheduler.to(CFG.device)
    
    print(model_state_dict['best_dice'])
    ...
    #= torch.nn.Sequential(torch.nn.Conv2d(CFG.in_chans, 3, kernel_size=1, bias=False),
    #smp.Unet('resnet18', encoder_weights='imagenet', decoder_use_batchnorm=False, decoder_attention_type='scse'))
...
if __name__ == "__main__":
    ...
    model_path = '/home/dmitry/Documents/KaggleCompetitions/Vesuvius/models/resnet_meanpool_18/fold_1/resnet_meanpool_18_fold_1_epoch_6_model.pth'
    retrain_on_fold(1, writer, model_path)
