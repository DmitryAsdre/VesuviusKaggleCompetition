...
from utils.image_loaders import get_train_valid_dataset, read_images_mask_pca_npy
...
def train_fn(train_loader, model, scheduler, criterion, optimizer, device):
    model.train()
...
def train_on_fold(valid_img, writer):    
    ...
    read_images_mask = partial(read_images_mask_pca_npy, PATH_TO_DS = CFG.PATH_TO_DS, chans_idxs=CFG.chans_idxs, tile_size=CFG.tile_size, surface_volume_name='inklabels_pca_10')
    train_images, train_masks, valid_images, valid_masks, valid_xyxys = get_train_valid_dataset(valid_img, read_images_mask, CFG.tile_size, CFG.stride, 4)
    ...
    optimizer = AdamW(model.parameters(), lr=CFG.lr, weight_decay=0.00005)
    scheduler = OneCycleLR(optimizer, max_lr=1e-4, epochs=CFG.epochs, steps_per_epoch=448)
    #scheduler = get_scheduler(CFG, optimizer)
    
    for epoch in range(CFG.epochs):
        
        loss = train_fn(train_dataloader, model, scheduler, criterion, optimizer, CFG.device)
