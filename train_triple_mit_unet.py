...
class CFG:
    ...
    PATH_TO_DS = '../data'
    PATH_TO_SAVE = '../models/'
    exp_name = 'triple_mit_l1_9_slices_from_the_middle'
    log_dir = '../tensorboard_logging'
...
def train_on_fold(valid_img, writer):    
    ...
    read_images_mask = partial(read_images_mask_middle_layers, PATH_TO_DS = '../data', chans_idxs=CFG.chans_idxs, tile_size=CFG.tile_size)
    train_images, train_masks, valid_images, valid_masks, valid_xyxys = get_train_valid_dataset(valid_img, read_images_mask, CFG.tile_size, CFG.stride)
    ...
    for epoch in range(CFG.epochs):
        ...
        for th in ths:
            writer.add_scalar(f'FOLD_{valid_img}/TH/th-{th}', ths[th], epoch)
            
        writer.add_scalar(f'FOLD_{valid_img}/Loss/train_loss', loss, epoch)
        writer.add_scalar(f'FOLD_{valid_img}/Loss/valid_loss', avg_val_loss, epoch)
        writer.add_scalar(f'FOLD_{valid_img}/Metrics/f_beta_best', best_dice, epoch)
            
        
        
        torch.save({'model' : model.state_dict(), 
                    'preds' : mask_pred},
                   os.path.join(CUR_PATH_TO_SAVE, f'{CFG.exp_name}_fold_{valid_img}_epoch_{epoch}.pth'))
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(log_dir=CFG.log_dir, comment=comment)
